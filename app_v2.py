...
""")
    ...
    st.caption("Search only the medicine name you are looking for.")

    medicine_search = st.text_input(
        "Search medicine",
        placeholder="Type 1st letter, e.g. P, A, C...",
        key="sidebar_medicine_search",
        label_visibility="collapsed",
    )

    search_clicked = st.button("🔎 Search Medicine", use_container_width=True)

    # Search is prefix-first: one letter is enough.
    # Example: P → Paracetamol, A → Amoxicillin/Azithromycin/Aspirin/...
    if search_clicked:
        query = medicine_search.strip().casefold()

        if not query:
            st.info("Pehle medicine ka first letter ya naam type karein.")
        else:
            # Normalize spaces/hyphens so small typing differences do not block results.
            normalized = {
            ...
            # Only use contains matching when there is no prefix match.
            ...
            if matches:
                st.caption("Verified medicines matching your search:")
                for medicine in matches[:8]:
                    if st.button(
                        f"💊 {medicine}",
                        key=f"medicine_result_{medicine}",
                        use_container_width=True,
                    ):
                        st.session_state.selected_medicine = medicine
                        st.rerun()
                if len(matches) > 8:
                    st.caption(f"Showing first 8 of {len(matches)} matches. Type more letters to narrow the search.")
            else:
                st.warning("Is letter/name se koi verified medicine nahi mili.")
...
"""
            ...
            user_content = (
                ...
                f"SELECTED MEDICINE:\n{selected_medicine}\n\n"
                f"USER HEALTH DETAILS (OPTIONAL):\n{health_details_context()}\n\n"
