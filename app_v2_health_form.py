...
# =========================================================
# OPTIONAL HEALTH DETAILS FORM
# =========================================================
def render_health_details_form():
    with st.expander("🩺 Add Health Details (Optional)", expanded=False):
        st.caption("These details help the assistant understand your question more safely. They do not create a diagnosis or prescription.")
        with st.form("health_details_form"):
            symptoms = st.multiselect(
                ...
                [
                    ...
                    "Body pain",
                    "Allergy",
                    "Chest discomfort / Heart-related concern",
                    "Breathing difficulty",
                    "Other",
                ],
                default=st.session_state.get("health_details", {}).get("symptoms", []),
            )
            duration = st.selectbox(
                "How long have you had this problem?",
                ["Not sure", "Today", "1–3 days", "4–7 days", "More than 1 week"],
                index=["Not sure", "Today", "1–3 days", "4–7 days", "More than 1 week"].index(
                    st.session_state.get("health_details", {}).get("duration", "Not sure")
                ),
            )
            severity = st.radio(
                "How severe is it?",
                ["Mild", "Moderate", "Severe", "Not sure"],
                index=["Mild", "Moderate", "Severe", "Not sure"].index(
                    st.session_state.get("health_details", {}).get("severity", "Not sure")
                ),
                horizontal=True,
            )
            conditions = st.multiselect(
                ...
                default=st.session_state.get("health_details", {}).get("conditions", []),
            )
            pregnancy = st.radio(
                "Pregnancy status (if relevant)",
                ["Not applicable", "Yes", "No", "Possibly", "Prefer not to say"],
                index=["Not applicable", "Yes", "No", "Possibly", "Prefer not to say"].index(
                    st.session_state.get("health_details", {}).get("pregnancy", "Not applicable")
                ),
            )
            current_medicines = st.text_input(
                "Are you currently taking any medicines? (Optional)",
                value=st.session_state.get("health_details", {}).get("current_medicines", ""),
            )
            allergies = st.text_input(
                "Any medicine allergy? (Optional)",
                value=st.session_state.get("health_details", {}).get("allergies", ""),
            )
            extra = st.text_area(
                "Anything else important? (Optional)",
                value=st.session_state.get("health_details", {}).get("extra", ""),
            )
        ...
        if submitted:
            st.session_state.health_details = {
                ...
                "pregnancy": pregnancy,
                "current_medicines": current_medicines.strip(),
                "allergies": allergies.strip(),
                "extra": extra.strip(),
            }
            st.success("Health details saved for this conversation.")
            st.rerun()

    details = st.session_state.get("health_details", {})
    if details and any(details.values()):
        st.caption("🩺 Health details added • You can reopen the form to update them.")
