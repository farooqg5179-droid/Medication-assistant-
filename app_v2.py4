...
def render_problem_form():
    ...
    if submitted:
        ...
        st.success("Health details saved. Ab AI aapke diye hue answers ko context mein use karega.")
        st.rerun()
...
""")
    ...
    medicine_search = st.text_input(
        "Search medicine",
        placeholder="Type e.g. P, Pa, Para...",
    ...
    if search_clicked:
        query = medicine_search.strip().lower()
        if not query:
            st.info("Pehle medicine ka naam ya first letters type karein.")
        else:
            prefix_matches = [
            ...
            if matches:
                st.caption(f"{len(matches)} matching medicine(s)")
                ...
                st.warning("Is naam/letter se koi verified medicine nahi mili.")
